...
- [ ] **AS ↔ DN 之間加滑軌拉門** — 公私區分隔（客廳 / 臥室衛浴側）
  ...
  - 門扇重量與上軌結構強度需評估
  - **拉門開啟時：面北時往左（西）收入冰箱縫隙（[AW](AW.md) 冰箱南側）** — AS 與 BN 整片牆保持完整，不需預留拉門收納槽
  - [ ] **需實際評估冰箱空間是否可行**（縫隙水平長度需容納整扇門 + 鏡面）
  - 關閉時拉門跨越 AS↔DN 位置
  - 另見 [DN 頁](DN.md) — 拉門 D 側整面穿衣鏡 + 補光 (B/C 共用更衣區)

### 客廳 ↔ 主臥 通風採光窗（AS 上段）

> AS 同時是 [B 主臥](BN.md) 的北牆 — 客廳與主臥共享的這面牆上方靠近天花板處，開一排通風採光窗，把客廳的光與空氣引入主臥，但保持主臥隱私與休息時的光/聲隔絕。

- [ ] **位置**：AS 靠天花板頂端開一排橫向高窗（high clerestory），高度避開站立視線
- [ ] **開窗尺寸**：因 B 側衣櫃在 [BW](../walls/BW.md) 而非 BN，AS 上段不被衣櫃擋住，**窗孔可加大**（採光/通風更充足）
