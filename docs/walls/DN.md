...
- [ ] 拉門與 [AS 頁](AS.md) 相連 — 分隔客廳與臥室 / 衛浴側
- [ ] **只做上方滑軌**（吊掛式），**不做地面軌道**
- [ ] **D 側 = 整面穿衣鏡**（full-length） — 同時作為 [B 房](../rooms/B.md) 與 [C 房](../rooms/C.md) 的**共用更衣區**
- [ ] **門開時 = 面北時拉門往左（西）收入 [AW 冰箱南側縫隙](AW.md)**（冰箱背靠 W、右開東），鏡面跟著收進縫隙，客廳與更衣區都看不到鏡子；**AS 與 BN 牆面完整保留**，不挖收納槽
- [ ] **需實際評估冰箱空間是否可行**（AW 冰箱南側到 AS 的水平距離是否夠門扇 + 鏡面整段收納）
...
- [ ] 縫隙淨寬 ≥ 門扇厚度 + 鏡面厚度（確認冰箱到 AS 牆的距離可容納）
- [ ] 上軌延伸進冰箱縫隙，頂端固定與承重（吊掛式門扇重量）
